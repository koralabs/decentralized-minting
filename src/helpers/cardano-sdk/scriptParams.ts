...
export const plutusV2ScriptHash = (cborHex: string): string => {
  ...
  // byte-string-of-byte-string: check if the inner byte (after the outer
  // byte-string header) is also a byte-string major type. If not, we have
  // single-CBOR and need to wrap once.
  const outerMt = bytes[0] >> 5;
  if (outerMt !== 2) {
    throw new Error("expected CBOR byte string as script input");
  }
  ...
  const innerFirstByte = bytes[headerLen];
  const innerMt = innerFirstByte >> 5;
  const isDoubleCbor = innerMt === 2;
  const doubleCborHex = isDoubleCbor ? cborHex : toDoubleCbor(cborHex);

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return (Serialization as any)
    .PlutusV2Script.fromCbor(doubleCborHex as HexBlob)
    .hash() as string;
};
