...
import { addValues, makeTxOutputId, TxInput } from "@helios-lang/ledger";
...
describe.sequential("Koralab Decentralized Minting Tests", () => {
  ...
  // user_1 orders legacy handle - <legacy-1>
  myTest(
    "user_1 orders legacy handle - <legacy-1>",
    async ({ network, emulator, wallets, ordersDetail }) => {
      invariant(Array.isArray(ordersDetail), "Orders detail is not an array");
  ...
  // user_1 orders legacy handle - <legacy-2>
  myTest(
    "user_1 orders legacy handle - <legacy-2>",
    async ({ network, emulator, wallets, ordersDetail }) => {
      invariant(Array.isArray(ordersDetail), "Orders detail is not an array");
  ...
  myTest(
    ...
    async ({
      ...
      const areAllLegacyHandles = orderedHandles.every(
        (handle) => handle.handle.isLegacy
      );
      invariant(areAllLegacyHandles, "Must mint only legacy handles");

      const txBuilderResult = await prepareLegacyMintTransaction({
        address: allowedMinter1Wallet.address,
        handles: orderedHandles.map((order) => order.handle),
        db,
        blockfrostApiKey: "",
      });
      invariant(txBuilderResult.ok, "Mint Tx Building Failed");
      ...
      orderedHandles.forEach((order) =>
        mintingHandlesTokensValue.push(
          [
            referenceAssetClass(legacyPolicyId, order.handle.utf8Name)
              .tokenName,
            1n,
          ],
          [userAssetClass(legacyPolicyId, order.handle.utf8Name).tokenName, 1n]
        )
      );
      ...
      orderedHandles.forEach((order) =>
        txBuilder
          .spendUnsafe(order.orderTxInput, buildOrderExecuteAsLegacyRedeemer())
          .payUnsafe(
            settingsV1.pz_script_address,
            referenceAssetValue(legacyPolicyId, order.handle.utf8Name)
          )
          .payUnsafe(
            user1Wallet.address,
            userAssetValue(legacyPolicyId, order.handle.utf8Name)
          )
      );
