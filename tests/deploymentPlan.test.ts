...
const desiredState: DesiredDeploymentState = {
  ...
  assignedHandles: {
    settings: ["demi@handle_settings", "demi_root@handle_settings", "kora@handle_prices"],
    scripts: ["demimntprx1@handlecontract", "demimntmpt1@handlecontract"],
  },
  ignoredSettings: ["settings.values.demi_root@handle_settings.mpt_root_hash"],
  settings: {
  ...
  it("decodes the live settings handles into comparable YAML-shaped values", async () => {
    const live = await fetchLiveSettingsState({
      ...
      fetchFn: vi.fn(async (url) => {
        ...
        if (target.endsWith("/handles/demi_root%40handle_settings")) {
          return new Response(JSON.stringify({
            utxo: "b".repeat(64) + "#0",
        ...
        if (target.endsWith("/handles/demi_root%40handle_settings/utxo")) {
        ...
        if (target.endsWith("/handles/demi_root%40handle_settings/datum")) {
    ...
    expect(live.currentSettingsUtxoRefs).toEqual({
      ...
      "demi_root@handle_settings": "b".repeat(64) + "#0",
    ...
    const plan = buildDeploymentPlan({
      ...
      liveSettings: {
        ...
        values: {
          ...
          "demi_root@handle_settings": { mpt_root_hash: "old-root" },
