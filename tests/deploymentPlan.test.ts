  ...
  it("decodes the live settings handles into comparable YAML-shaped values", async () => {
    const live = await fetchLiveSettingsState({
      ...
      fetchFn: vi.fn(async (url) => {
        ...
        if (target.endsWith("/handles/hal%40handle_settings")) {
        ...
        if (target.endsWith("/handles/hal_root%40handle_settings")) {
          return new Response(JSON.stringify({
            utxo: "b".repeat(64) + "#0",
        ...
        if (target.endsWith("/handles/hal_root%40handle_settings/utxo")) {
        ...
        if (target.endsWith("/handles/hal%40handle_settings/datum")) {
        ...
        if (target.endsWith("/handles/hal_root%40handle_settings/datum")) {
