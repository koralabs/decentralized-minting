...
- [x] `DSH-403` (pkg) **Mint build relocation.** Build DeMi subhandle orders on the orders/`MintNewHandles` path (not legacy `prepareLegacyMint`); construct free-virtual proofs (root proof taken AFTER the sub key insert + current `free_names`); trie maintenance — insert the sub key AND bump the root key's `free_names`. deps: DSH-401, DSH-402 — **done 70dc792**: `src/txs/orderProofs.ts buildOrderProofs` + `mintNew` virtual(000→pz)/nft(100→pz+222→dest) branch + `freeVirtualContexts` engine hook; in-memory-Trie unit tests (hash-equivalence). Fee folding/metadata = DSH-501.
...
- [x] `DSH-406` (pkg) **Blueprints + config.** Regenerate optimized/unoptimized blueprints (ABI changed → new `demimntmpt`/`demimnt` hashes); update the pinned-hash test + `deploy/preview` config (`minting_data_script_hash`, `mint_governor`). deps: DSH-401, DSH-202 — **done 0356588**: rebuilt w/ aiken v1.0.29-alpha+16fb02e; demimntmpt(ABI)+demimnt(DSH-201) hashes changed, demimntprx/demiord stable; preview applied hashes recomputed via buildContracts; aiken check 170/0/0, 66 vitest. Stale deploy/preview/*.unoptimized.cbor deferred to DSH-602 bundle regen.
