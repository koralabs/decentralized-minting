...
| DSH-403 | in_progress | DSH-401, DSH-402 | decentralized-minting pkg | 2026-06-06T02:10:00Z | — | — | mint build relocation to orders/MintNewHandles + free-virtual proofs + trie maintenance. Contract semantics (utils.ak all_orders_are_satisfied): sub key insert value=#"" (mpt.insert); free private virtual = 2nd mpt.update on ROOT key (old=encode(free_names,labels), new=encode(addFreeName(free_names,sub),labels), root_proof AFTER sub insert); virtual mints 000→pz (1 output), nft/root mints 100→pz+222→dest (2 outputs). Scope=proofs+trie+token mint/outputs; fee folding/metadata=DSH-501 |
